...
// 1. INJEKSI SVGS KUSTOM DARI PROMPT ANDA
const BackgroundPattern = ({ className }) => (
    <svg className={className} id="Layer_1" data-name="Layer 1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1920.05 206.77" preserveAspectRatio="none">
        ...
        <path className="cls-1" d="M377.6,174.71V.38H32v174.34h377.6s-32-.01-32-.01ZM220.8,32.05h-32s32-.01,32-.01h h0Z"/>
...
// 2. MAIN SKOR KONTROLER
function App() {
    const [view, setView] = useState('dashboard'); // Pilihan: dashboard, admin, peserta
    const [currentTime, setCurrentTime] = useState(new Date());
    const [sisaDetik, setSisaDetik] = useState(620); // Default countdown: 10 mnt 20 dtk
    const [searchKeyword, setSearchKeyword] = useState('');
    
    const [db, setDb] = useState({ config: {}, modules: [], schedule: [] });
    const [peserta, setPeserta] = useState(window.DATA_PESERTA || []);
    ...
    useEffect(() => {
        fetch('data.json')
            ...
            .catch(err => console.error("Error muat JSON: ", err));
    }, []);
    ...
    // Trigger keyword rahasia 'admin123'
    useEffect(() => {
        if (searchKeyword.toLowerCase() === 'admin123') {
            setView('admin');
            setSearchKeyword('');
        }
    }, [searchKeyword]);

    // Format utility untuk hitung mundur
        ...
        <div className="flex flex-col h-screen w-screen overflow-hidden bg-white">
            ...
            <header className="h-[20vh] relative w-full border-b-4 border-lks-blue shrink-0">
                ...
                <div className="absolute inset-0 flex items-center justify-between px-12 z-10">
                    <div className="bg-white/95 p-2 rounded-xl border border-slate-200 shadow-sm max-h-[75%] flex items-center">
                        <LogoGdtSkills className="h-10 md:h-12 w-auto" />
                    </div>
                    <div className="bg-white/95 px-8 py-3 rounded-xl border border-slate-200 shadow-sm text-center">
            ...
            {/* --- BARIS 2, 3, 4: CENTER CONTENT (3/5 TINGGI) --- */}
            <main className="h-[60vh] w-full p-8 bg-slate-50/40 shrink-0 overflow-y-auto">
                {view === 'dashboard' && (
                    <div className="w-full h-full flex flex-col md:flex-row gap-8">
                        {/* TIMER KIRI (75% Lebar) */}
                        <div className="w-full md:w-3/4 bg-white border border-slate-200/60 rounded-2xl flex flex-col items-center justify-center relative shadow-sm">
                            <h2 className="text-xl md:text-3xl font-extrabold text-lks-pink mb-2 animate-pulse">&lt;&lt;Agenda Saat Ini&gt;&gt;</h2>
                            <div className="text-7xl sm:text-8xl md:text-9xl font-black text-lks-blue font-mono tracking-tight select-none">
                                {hours}:{minutes}:{seconds}
                            </div>
                            <p className="text-sm md:text-base font-bold text-lks-pink tracking-widest mt-4">Jam : Menit : Detik</p>
                        </div>

                        {/* WIDGET KANAN (25% Lebar) */}
                        <div className="w-full md:w-1/4 flex flex-col justify-between gap-4">
                            <div>
                                ...
                                <div className="bg-lks-green text-white p-4 rounded-xl font-bold border border-emerald-600">
                                    <div className="text-xl font-black font-mono">{currentTime.toLocaleTimeString('id-ID')} WIB</div>
                                    <div className="text-[11px] text-emerald-100 mt-0.5">{currentTime.toLocaleDateString('id-ID', {day: 'numeric', month: 'long', year: 'numeric'})}</div>
                                </div>
                            </div>
                            <div className="flex-1 flex flex-col">
                                <span className="text-xs font-bold text-lks-blue block mb-1">Agenda Selanjutnya:</span>
                                <div className="bg-lks-green text-white p-4 rounded-xl font-extrabold border border-emerald-600 flex-1 flex items-start">
                                    <p className="text-base leading-snug">{db.schedule[0]?.title || "Sesi Pengumpulan Tugas"}</p>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {/* VIEW: FOLDER DAFTAR PESERTA */}
                {view === 'peserta' && (
                    <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm max-h-full overflow-y-auto">
                        <div className="flex justify-between items-center border-b pb-3 mb-4">
                            <h3 className="text-lg font-bold text-lks-blue">Folder Drive Pengumpulan Peserta</h3>
                            <button onClick={() => setView('dashboard')} className="text-xs font-bold bg-slate-100 px-3 py-1.5 rounded-lg">Kembali</button>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {peserta.map((p, idx) => (
                                <div key={idx} className="p-3 bg-slate-50 border rounded-xl flex items-center justify-between text-sm">
                                    <div>
                                        <span className="font-mono bg-lks-blue/10 text-lks-blue px-2 py-0.5 rounded font-bold mr-2">{p.no}</span>
                                        <span className="font-bold text-slate-700">{p.nama}</span>
                                    </div>
                                    <a href={p.link} target="_blank" rel="noreferrer" className="bg-lks-blue text-white px-3 py-1 rounded-lg text-xs font-bold shadow">Buka Folder</a>
                                </div>
                            ))}
                        </div>
                    </div>
                ...
                {view === 'admin' && (
                    <div className="bg-white rounded-2xl border-2 border-lks-pink p-6 max-w-xl mx-auto shadow-md">
                        <div className="flex justify-between items-center border-b pb-2 mb-4">
                            <h2 className="font-black text-lks-pink text-base uppercase tracking-wide">Admin Control Panel</h2>
                            <button onClick={() => setView('dashboard')} className="text-xs font-bold bg-slate-100 px-2.5 py-1 rounded-md">Tutup</button>
                        </div>
                        <div className="space-y-4 text-xs">
                            <div>
                                <label className="block font-bold text-slate-600 mb-1">Set Durasi Mundur (Menit):</label>
                                <input type="number" defaultValue={Math.floor(sisaDetik/60)} onChange={(e) => setSisaDetik((parseInt(e.target.value) || 0) * 60)} className="w-full px-3 py-2 border rounded-xl bg-slate-50 font-bold focus:outline-none"/>
                            </div>
                            <div className="pt-2">
                                <span className="block font-bold text-slate-400 mb-2">Simulasi Deteksi Input Trigger:</span>
                                <input type="text" placeholder="Ketik kata kunci di sini..." value={searchKeyword} onChange={(e) => setSearchKeyword(e.target.value)} className="w-full px-3 py-2 border rounded-xl text-center text-sm font-mono focus:outline-none bg-slate-50"/>
                            </div>
                        </div>
                    </div>
                )}
            ...
            {/* --- BARIS 5: FOOTER NAVIGATION (1/5 TINGGI) --- */}
            <footer className="h-[20vh] relative w-full border-t-4 border-lks-blue shrink-0">
                ...
                <div className="absolute inset-0 flex items-center justify-center gap-4 px-12 z-10">
                    <button onClick={() => setView('dashboard')} className="bg-white border-2 border-lks-blue text-lks-blue font-extrabold px-6 py-2.5 rounded-xl shadow-sm hover:scale-105 transition text-sm">
                        Dashboard Utama
                    </button>
                    <button onClick={() => setView('peserta')} className="bg-white border-2 border-lks-blue text-lks-blue font-extrabold px-6 py-2.5 rounded-xl shadow-sm hover:scale-105 transition text-sm">
                        Folder Pengumpulan
                    </button>
                    <div className="hidden lg:block bg-white/80 px-4 py-2 rounded-xl text-[11px] font-bold text-slate-400 border border-slate-200">
                        Ketik <code className="text-lks-pink font-mono bg-slate-100 px-1 py-0.5 rounded">admin123</code> untuk CMS Control
                    </div>
                </div>
            </footer>
