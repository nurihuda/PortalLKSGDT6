...
// --- LOGO GDTSKILLS DENGAN PNG TRANSPARAN ---
const LogoGdtSkills = () => (
    <img 
        src="logo.png" 
        alt="GDT Skills Logo" 
        ...
        <div className="flex flex-col h-screen w-screen overflow-hidden font-karla transition-colors duration-200" style={{ backgroundColor: bgMain }}>
            ...
            <header className="h-[16.6vh] w-full shrink-0 flex items-center justify-between px-12 lg:px-24 relative z-20 transition-colors duration-200" style={{ backgroundColor: bgHeader }}>
                <button 
                    ...
                    className="flex items-center justify-start transition hover:opacity-90 focus:outline-none cursor-pointer border-0 bg-transparent p-0"
                >
                    <LogoGdtSkills />
                </button>
                ...
                {view === 'dashboard' && (
                    <div className="w-full h-full flex flex-col md:flex-row gap-8">
                        ...
                        <div ref={timerRef} className={`rounded-none flex flex-col items-center justify-center relative shadow-sm transition-colors duration-200 ${isFullscreen ? 'w-screen h-screen fixed inset-0 z-50 p-12' : 'w-full md:w-3/4'}`} style={{ backgroundColor: bgCard }}>
                            ...
                            <p className={`font-bold text-[#e38ebc] tracking-widest select-none ${isFullscreen ? 'text-2xl mt-6' : 'text-xs md:text-sm mt-2'}`}>
                                Jam : Menit : Detik
                            </p>
                        </div>
                        ...
                        {!isFullscreen && (
                            <div className="w-full md:w-1/4 flex flex-col justify-between gap-4">
                                ...
                                <div 
                                    className="p-6 font-bold flex flex-col justify-center h-1/2 rounded-none shadow-sm" 
                                    style={{ backgroundColor: '#03a550', color: '#ffffff' }}
                                >
                                    <span className="text-xs font-bold block mb-2 opacity-90 uppercase tracking-wider" style={{ color: '#e6f7ed' }}>Waktu Sistem (WIB):</span>
                                    <div className="text-3xl lg:text-4xl font-black font-mono leading-none tracking-tight" style={{ color: '#ffffff' }}>
                                        {timeFormatter.format(dateObj).replace(/\./g, ':')}
                                    </div>
                                    <div className="text-xs mt-2 opacity-90" style={{ color: '#e6f7ed' }}>
                                        {dateFormatter.format(dateObj)}
                                    </div>
                                </div>

                                {/* KOTAK AGENDA SELANJUTNYA (BG BIRU #2982c5 + TEKS PUTIH) */}
                                <div 
                                    className="p-6 font-bold flex flex-col justify-center h-1/2 rounded-none shadow-sm" 
                                    style={{ backgroundColor: '#2982c5', color: '#ffffff' }}
                                >
                                    <span className="text-xs font-bold block mb-2 opacity-90 uppercase tracking-wider" style={{ color: '#e0f2fe' }}>Agenda Selanjutnya:</span>
                                    <p className="text-lg lg:text-xl leading-snug font-extrabold" style={{ color: '#ffffff' }}>
                                                    ...
                                                    <tr key={s.id} className={`transition-colors border-b border-slate-200 ${isNow ? 'bg-sky-100 font-bold' : ''}`} style={{ backgroundColor: isNow ? (isDarkMode ? '#0c4a6e' : '#e0f2fe') : bgCard }}>
                                                        ...
                                                        <td className="py-4 px-6 text-center whitespace-nowrap">
                                                            {isNow ? (
                                                                ...
                                                                <span className="bg-white text-[#2982c5] border border-slate-200 px-4 py-1.5 text-xs font-bold rounded-full">Akan Datang</span>
                                                            )}
                                                        </td>
                                                    </tr>
                                                )
