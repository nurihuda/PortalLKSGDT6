...
// 1. INJEKSI BACKGROUND PATTERN & LOGO SVG (TANPA ROUNDED/SHADOW)
...
function App() {
    const [view, setView] = useState('dashboard'); // dashboard, modules, peserta, schedule, links, admin
    ...
    // Pemicu Admin Area Rahasia via State Search Input
    useEffect(() => {
    ...
    // Cari Modul Aktif Rilis Otomatis Berdasarkan Waktu Sistem
    const modulTerbukaOtomatis = useMemo(() => {
        return db.modules.filter(m => currentTime >= new Date(m.releaseTime));
    }, [db.modules, currentTime]);

    // Data Filter dan Shorting Folder Peserta
        ...
        <div className="flex flex-col h-screen w-screen overflow-hidden bg-white font-karla">
            ...
            <header className="h-[20vh] relative w-full border-b-4 border-lks-blue shrink-0">
                ...
                <div className="absolute inset-0 flex items-center justify-between px-12 z-10">
                    <div className="bg-white p-2 border-0 shadow-none max-h-[75%] flex items-center">
                        <LogoGdtSkills className="h-10 md:h-12 w-auto" />
                    </div>
                    <div className="bg-white px-8 py-3 border-0 shadow-none text-center">
            ...
            {/* --- BARIS 2, 3, 4: MAIN DASHBOARD AREA (3/5 TINGGI) --- */}
            <main className="h-[60vh] w-full p-8 bg-slate-50/40 shrink-0 overflow-y-auto">
                ...
                {view === 'dashboard' && (
                    <div className="w-full h-full flex flex-col md:flex-row gap-8">
                        {/* TIMER KIRI (75% LEBAR) -> PURE KOTAK, NO ROUNDED, NO OUTLINE */}
                        <div className="w-full md:w-3/4 bg-white border-0 rounded-none flex flex-col items-center justify-center relative shadow-none">
                            <h2 className="text-xl md:text-3xl font-extrabold text-lks-pink mb-1 select-none tracking-wide">&lt;&lt;Agenda Saat Ini&gt;&gt;</h2>
                            <div className="text-[280pt] font-bold text-lks-blue font-karla leading-none tracking-tighter select-none select-none">
                                {hours}:{minutes}:{seconds}
                            </div>
                            <p className="text-sm md:text-base font-bold text-lks-pink tracking-widest mt-2">Jam : Menit : Detik</p>
                        </div>

                        {/* WIDGET KANAN (25% LEBAR) -> PURE KOTAK WARNA HIJAU TEKS PUTIH SAJA */}
                ...
                {/* VIEW: 1. AKSES MODUL TERBUKA OTOMATIS */}
                {view === 'modules' && (
                    <div className="bg-white rounded-none border-0 p-6 shadow-none max-h-full overflow-y-auto">
                        <div className="flex justify-between items-center border-b pb-3 mb-4">
                            ...
                            <span className="text-xs font-bold text-slate-400">Rilis Otomatis Sesuai Waktu Server</span>
                        </div>
                            ...
                            {db.modules.map((m) => {
                                ...
                                return (
                                    <div key={m.id} className="p-4 bg-slate-50 border-0 rounded-none flex items-center justify-between">
                                        ...
                                        {isReleased ? (
                                            <a href={m.link} target="_blank" rel="noreferrer" className="bg-lks-blue text-white px-4 py-2 rounded-none text-xs font-bold transition-all">Download Soal</a>
                                        ) : (
                ...
                {/* VIEW: ADMIN CONFIGURATION MODAL */}
                {view === 'admin' && (
            ...
            {/* --- BARIS 5: FOOTER (1/5 TINGGI MEMENTO KOTAK PUTIH 40PT RAKSASA) --- */}
            <footer className="h-[20vh] relative w-full border-t-4 border-lks-blue shrink-0">
                ...
                <div className="absolute inset-0 flex items-center justify-center gap-6 px-12 z-10 w-full h-full py-2">
                    
                    {/* BUTTON 1: AKSES MODUL */}
                    <button onClick={() => setView('modules')} className={`bg-white border-0 text-slate-900 font-semibold rounded-none shadow-none h-[85%] w-1/4 flex items-center justify-center text-[40pt] hover:bg-slate-50 active:scale-[0.99] transition-all cursor-pointer ${view === 'modules' ? 'bg-slate-100 ring-2 ring-lks-blue' : ''}`}>
                        Akses Modul
                    </button>

                    {/* BUTTON 2: FOLDER PENGUMPULAN */}
                    <button onClick={() => setView('peserta')} className={`bg-white border-0 text-slate-900 font-semibold rounded-none shadow-none h-[85%] w-1/4 flex items-center justify-center text-[40pt] hover:bg-slate-50 active:scale-[0.99] transition-all cursor-pointer ${view === 'peserta' ? 'bg-slate-100 ring-2 ring-lks-blue' : ''}`}>
                        Folder Pengumpulan
                    </button>

                    {/* BUTTON 3: JADWAL KOMPETISI */}
                    <button onClick={() => setView('schedule')} className={`bg-white border-0 text-slate-900 font-semibold rounded-none shadow-none h-[85%] w-1/4 flex items-center justify-center text-[40pt] hover:bg-slate-50 active:scale-[0.99] transition-all cursor-pointer ${view === 'schedule' ? 'bg-slate-100 ring-2 ring-lks-blue' : ''}`}>
                        Jadwal Kompetisi
                    </button>

                    {/* BUTTON 4: LINK LINK PENTING */}
                    <button onClick={() => setView('links')} className={`bg-white border-0 text-slate-900 font-semibold rounded-none shadow-none h-[85%] w-1/4 flex items-center justify-center text-[40pt] hover:bg-slate-50 active:scale-[0.99] transition-all cursor-pointer ${view === 'links' ? 'bg-slate-100 ring-2 ring-lks-blue' : ''}`}>
                        Link Link Penting
                    </button>
